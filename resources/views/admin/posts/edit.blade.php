	...
	<div class="bg-white border border-red-200 shadow-2xl rounded-lg mb-8">
		...
		<form method="POST" action="{{ route('admin.posts.update', $post) }}" enctype="multipart/form-data" class="p-6 space-y-6">
			...
			<div>
				...
				<textarea name="content" id="content" rows="10" class="bg-white border border-red-200 text-gray-900 rounded-lg px-4 py-2 w-full focus:ring-2 focus:ring-red-500 focus:border-red-500 rich-editor" required>{{ old('content', $post->content) }}</textarea>
			</div>
			<div>
